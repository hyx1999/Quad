...
def add_lora_in_linear(module, no_lora: bool = False):
    def get_fwd_fn():
    ...
    fwd_fn = get_fwd_fn()
    if no_lora:
        setattr(module, "lora_forward", MethodType(lambda self, input: 0, module))
    else:
...
class QuadQuantableQwen2Attention(Qwen2Attention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config.svd_rank > 0:
            ...
            add_lora_in_linear(self.o_proj, no_lora=True)
...
class QuadQuantableQwen2MLP(Qwen2MLP):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        if config.svd_rank > 0:
            ...
            add_lora_in_linear(self.down_proj, no_lora=True)
