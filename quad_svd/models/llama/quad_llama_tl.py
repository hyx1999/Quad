...
from quad_svd import TensorPack
from quad_svd.modules import QuantLinearW4A4Tl, QuantLinearW4A8Tl, QuantizerTl
...
class LinearTypeMixin:
    ...
    def get_linear_type(self):
        if self.config.quant_mode == QuantMode.w4a4:
            QuantLinearU = QuantLinearW4A4Tl
            QuantLinearD = QuantLinearW4A4Tl
        elif self.config.quant_mode == QuantMode.w4a8:
            QuantLinearU = QuantLinearW4A8Tl
            QuantLinearD = QuantLinearW4A8Tl
        else:
            QuantLinearU = QuantLinearW4A4Tl
            QuantLinearD = QuantLinearW4A8Tl
...
class QuadLlamaAttention(LlamaFlashAttention2, LinearTypeMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        config: QuadLlamaConfig = self.config
        ...
        self.o_proj = nn.Sequential(
            QuantizerTl(config.hidden_size, 0, config.svd_rank, config.input_clip_ratio, act_dtype=actD),
            QLinearD.from_float(self.o_proj, extra_out=config.pod_rank, svd_rank=config.svd_rank)
        )
...
class QuadLlamaMLP(LlamaMLP, LinearTypeMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.down_proj = torch.nn.Sequential(
            QuantizerTl(config.intermediate_size, 0, config.svd_rank, config.input_clip_ratio, act_dtype=actD),
...
class QuadLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config):
        ...
        for layer_idx, layer in tqdm(enumerate(self.model.layers), total=len(self.model.layers), desc="init model..."):
            layer.self_attn = QuadLlamaAttention(config=config, layer_idx=layer_idx)
            ...
            layer.mlp = QuadLlamaMLP(config=config)
