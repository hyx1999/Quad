...
class QuadFp16LlamaConfig(LlamaConfig):
    model_type = "llama_quad_fp16"
...
class QuadFp16LlamaAttention(LlamaFlashAttention2):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        config: QuadFp16LlamaConfig = self.config
        ...
        self.q_proj = QuantLinearFp16.from_float(self.q_proj, pod_rank=config.pod_rank, svd_rank=config.svd_rank, lora_idx=0)
        self.k_proj = QuantLinearFp16.from_float(self.k_proj, pod_rank=config.pod_rank, svd_rank=config.svd_rank, lora_idx=1)
        self.v_proj = QuantLinearFp16.from_float(self.v_proj, pod_rank=config.pod_rank, svd_rank=config.svd_rank, lora_idx=2)
        self.o_proj_hadamard = quad.modules.OnlineHadamard(self.num_heads)
        self.o_proj = nn.Sequential(
            Identity(config.hidden_size, 0, svd_rank=config.svd_rank, num_adapters=1),
            QuantLinearFp16.from_float(self.o_proj, extra_out=config.pod_rank, svd_rank=config.svd_rank)
        )
...
class QuadFp16LlamaMLP(LlamaMLP):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.up_proj = QuantLinearFp16.from_float(
            self.up_proj, pod_rank=config.pod_rank, svd_rank=config.svd_rank, lora_idx=0)
        self.gate_proj = QuantLinearFp16.from_float(
            self.gate_proj, pod_rank=config.pod_rank, svd_rank=config.svd_rank, lora_idx=1)
        self.down_proj = torch.nn.Sequential(
            quad.modules.OnlineHadamard(self.intermediate_size),
            Identity(config.hidden_size, 0, svd_rank=config.svd_rank, num_adapters=1),
            QuantLinearFp16.from_float(self.down_proj, extra_out=config.pod_rank, svd_rank=config.svd_rank,)
        )
...
class QuadFp16LlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config):
        ...
        for layer_idx, layer in enumerate(self.model.layers):
            layer.self_attn = QuadFp16LlamaAttention(config=config, layer_idx=layer_idx)
            ...
            layer.mlp = QuadFp16LlamaMLP(config=config)
