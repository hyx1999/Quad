...
def apply_exact_had_to_linear(module, had_dim=-1, output=False):
    ...
    if hasattr(module, "adapters"):
        if had_dim == -1:
            if output:
                had_K, K = get_hadK(out_features)
                lora_A_ = module.adapters["lora_A"].weight.data.float().cuda()
                lora_A_ = matmul_hadU_cuda(lora_A_.t(), had_K, K).t()
                module.adapters["lora_A"].weight.data = lora_A_.to(device=dev, dtype=dtype)
            if not output:
                lora_B_ = module.adapters["lora_B"].weight.data.float().cuda()
                had_K, K = get_hadK(in_features)
                lora_B_ = matmul_hadU_cuda(lora_B_, had_K, K)
                module.adapters["lora_B"].weight.data = lora_B_.to(device=dev, dtype=dtype)
        else:
            if output:
                lora_A_ = module.adapters["lora_A"].weight.data.float().cuda()
                lora_A_ = lora_A_.t()
                transposed_shape = lora_A_.shape
                lora_A_ = fast_hadamard_transform.hadamard_transform(
                    lora_A_.reshape(-1, transposed_shape[-1]//had_dim, had_dim), 
                    scale=1/math.sqrt(had_dim)
                ).reshape(transposed_shape).t()
                module.adapters["lora_A"].weight.data = lora_A_.to(device=dev, dtype=dtype)
            else:
