...
class GPTQ:
    ...
    def __init__(self, layer, full_size):
        self.layer = layer
        self.dev = self.layer.weight.device
        W = layer.weight.data.clone()
        self.rows = W.shape[0]
        self.columns = W.shape[1] - full_size
    ...
    def fasterquant(
        ...
        W = self.layer.weight.data.clone()[:, self.full_size:]
        W = W.float()
        ...
        Q = Q.to(self.layer.weight.data.dtype)
        Q = torch.cat((self.layer.weight.data[:, :self.full_size], Q), dim=-1)
        assert Q.shape == self.layer.weight.shape
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                if "o_proj" in name or "down_proj" in name:
                    ...
                    full_size = args.pod_rank
                gptq[name] = GPTQ(subset[name], full_size)
...
def rtn_fwrd(model, dev, args):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            ...
            W = subset[name].weight.data[:, full_size:]
            quantizer.find_params(W)
            Q = quantizer.quantize(W).to(
                next(iter(layer.parameters())).dtype)
            Q = torch.cat((subset[name].weight.data[:, :full_size], Q), dim=-1)
