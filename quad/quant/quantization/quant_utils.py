...
class ActQuantWrapper(torch.nn.Module):
    ...
    def __init__(self, module:torch.nn.Linear):
        ...
        # Quad params
        self.expand_size = 0
        self.register_buffer('expand_had_K', torch.tensor(0))
        self._buffers['expand_had_K'] = None
        self.expand_K = 1
    ...
    def forward(self, x):
        ...
        if self.online_full_had:
            ...
            if self.expand_size > 0:
                x0, x1 = x[..., :self.expand_size], x[..., self.expand_size:]
                if self.fp32_had: # Full Hadamard in FP32
                    x1 = hadamard_utils.matmul_hadU_cuda(x1.float(), self.had_K, self.K).to(x_dtype)
                else: # Full Hadamard in FP16
                    x1 = hadamard_utils.matmul_hadU_cuda(x1, self.had_K, self.K)
        ...
        if hasattr(self.module, "lora_forward"):
            x_lora = self.module.lora_forward(x)
        else:
            x_lora = None

        if self.quantizer.bits < 16: #Quantize, if needed
            if self.mix_precision:
                x_full  = x[..., :self.full_size]
                x_quant = x[..., self.full_size:]
                self.quantizer.find_params(x_quant)
                x_quant = self.quantizer(x_quant).to(x_dtype)
                self.quantizer.free()
                x = torch.cat((x_full, x_quant), dim=-1)
            else:
