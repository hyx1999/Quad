...
def config_logging(log_file, level=logging.INFO):
    ...
    logger.setLevel(level)  # 设置全局日志级别为 INFO

    # 第二步：创建 handler，分别用于控制台和文件
    ...
    # 第三步：创建 formatter 并将其添加到 handler
    formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    console_handler.setFormatter(formatter)
    file_handler.setFormatter(formatter)

    # 第四步：将 handler 添加到 logger
...
def parser_gen():
    ...
    parser.add_argument('--int8_down_proj_act', action=argparse.BooleanOptionalAction, default=False,
                        help='Use INT8 for Down Projection Input Activation!')
    parser.add_argument('--int8_o_proj_act', action=argparse.BooleanOptionalAction, default=False,
                        help='Use INT8 for O Projection Input Activation!')
    ...
    parser.add_argument('--save_dequant_model_path', type=str, default=None, 
                        help='Save the dequant model to the specified path!')
    ...
    parser.add_argument("--load_svd_path", type=str, default=None)
    ...
    setattr(args, 'save_path',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'misc', 'experiments', model_name))

    os.makedirs(args.save_path, exist_ok=True)

    args.logger = config_logging(os.path.join(args.save_path, f'{args.save_name}.log'))
    ...
    args.logger.info(pprint.pformat(vars(args)))
    args.logger.info('--' * 30)
    return args
