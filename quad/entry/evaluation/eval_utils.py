...
def evaluator(model, testenc, dev, args):
    ...
    for i in tqdm(range(len(layers)), desc="(Eval) Layers"):
        ...
        if args.capture_layer_io and args.layer_idx == i:
            captured_io = module_utils.capture_layer_io(module_utils.get_model_type(model), layer, inps)
            save_path = module_utils.get_layer_io_save_path(args)
