...
def main():
    ...
    args.logger.info("save_path: {}".format(args.save_path))
    ...
    if args.rotate:
        ...
        for name in qlayers:
            if "o_proj" in name or "down_proj" in name:
                ...
                qlayers[name].mix_precision = True
                qlayers[name].full_size = args.pod_rank
    else:
    ...
    if args.a_bits < 16 or args.v_bits < 16:
        ...
        for name in qlayers:            
            ...
            if 'o_proj' in name: #Set the down_proj precision
                if args.int8_o_proj_act:
                    layer_input_bits = 8
            
            if 'down_proj' in name: #Set the down_proj precision
                if args.int8_down_proj_act:
                    layer_input_bits = 8
    ...
    if args.k_bits < 16:
        if args.k_pre_rope:
            ...
            rope_function_name = module_utils.get_rope_function_name(model)
    ...
    args.logger.info("task_names: {}".format(task_names))
    ...
    for task, result in results.items():
        if 'acc_norm,none' in result:
            metric_vals[task] = round(result.get('acc_norm,none'), 4)
        elif 'acc,none' in result:
            metric_vals[task] = round(result.get('acc,none'), 4)
