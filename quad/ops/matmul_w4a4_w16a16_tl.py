...
def tl_fuse_w4a4_w16a16_matmul(  # INT4 x INT4 (K >> 64) + FP16 x FP16 (K <= 64)
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bn, bm):
            ...
            Ar_shared = T.alloc_shared(Ar_shared_shape, r_dtype, scope=shared_scope)
            Br_shared = T.alloc_shared(Br_shared_shape, r_dtype, scope=shared_scope)
...
def get_fuse_w4a4_w16a16_matmul_kernel(
    in_features: int,
    out_features: int, 
    rank: int,
):
    ...
    if kernel_cache[(in_features, out_features, rank)] is None:
        print("init tl_fuse_w4a4_w16a16_matmul kernel...")
        program = tl_fuse_w4a4_w16a16_matmul(
